...
| `first_question_id`   | `string` | The ID of the first question (use as `question_id` for the first `/interview/respond`). |
...
| `next_question_id` | `string`  | The ID of the follow-up question (empty if session complete).  |
